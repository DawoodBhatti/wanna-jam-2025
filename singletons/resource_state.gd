...
func _ready() -> void:
	if SignalBus:
		# Hook into resource count requests
...
func _on_resource_count_started() -> void:
	# Called at end-of-turn before advancing
	print("[ResourceState] Reporting current resources")
	# Could also emit all resource_changed for sync:
